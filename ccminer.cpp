...
/*	if (work->data[0] & POK_BOOL_MASK) {
	...
	if (opt_algo == ALGO_DECRED) {
		...
		if (!have_longpoll && work->height > net_blocks + 1) {
			...
			applog(LOG_BLUE, "Hcash block %d%s", work->height, netinfo);
...
int share_result(int result, int pooln, double sharediff, const char *reason)
{
	...
	applog(LOG_NOTICE, "[A:%lu,R:%lu,T:%lu] diff: %2.3f, %s %s", p->accepted_count, p->rejected_count, p->accepted_count + p->rejected_count, sharediff, s, flag);
	if (reason) {
...
int main(int argc, char *argv[])
{
	...
	printf("*** " PROGRAM_NAME " " PACKAGE_VERSION " for nVidia GPUs from hcash.beepool.org ***\n");
#ifdef _MSC_VER
	...
	printf("*** Include some of the work of djm34, sp, tsiv, klausT and oldminer.\n\n");
