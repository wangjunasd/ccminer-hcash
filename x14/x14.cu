...
extern void x14_sm3_cpu_hash_64(int thr_id, uint32_t threads, uint32_t *d_hash);
...
extern "C" void x14hash(void *output, const void *input)
{
	...
	memset(hash, 0, sizeof hash);
	sph_sm3_close(&ctx_sm3, (void*)hash);
...
extern "C" int scanhash_x14(int thr_id,  struct work* work, uint32_t max_nonce, unsigned long *hashes_done){
	...
	uint32_t throughput = cuda_default_throughput(thr_id, 1<<20); // 19=256*256*8;
	...
	do {
		...
		x14_sm3_cpu_hash_64(thr_id, throughput, d_hash[thr_id]);
